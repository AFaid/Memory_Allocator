...
void bstree_delete(BStree bst, Key key) {
	if(bst->root == 0) printf("Empty Tree");
...
static Data *bst_search(BStree bst, int index, Key key) {
	...
	if(comp > 0) {
		if(bst->tree_nodes[index].left != 0) bst_search(bst, bst->tree_nodes[index].left, key);
		else return NULL;
	}
	else if(comp < 0) {
		if(bst->tree_nodes[index].right != 0) bst_search(bst, bst->tree_nodes[index].right, key);
		else return NULL;
	}

	else {
		Data* data_ptr = &(bst->tree_nodes[index].data);
		return data_ptr;
	}
}
...
//		  'index': index of current tree node
// 	      'Data': a data value
// Effect: Return the address of the key of the first node, in order of the keys in bst,
// 		   such that the data of the node is greater than or equals to data.
...
static Key* bst_data_search(BStree bst, int index, Data data) {
	...
	if(index==0){
		...
		key_ptr = bst_data_search(bst, bst->tree_nodes[index].left, data);

		if(key_ptr) 
			return key_ptr;
		else if(bst->tree_nodes[index].data >= data && key_ptr == NULL) 
			return &(bst->tree_nodes[index].key);
		else if(bst->tree_nodes[index].data < data && key_ptr == NULL) 
			key_ptr = bst_data_search(bst,bst->tree_nodes[index].right, data);

		if(key_ptr) return key_ptr;
		else return NULL;
